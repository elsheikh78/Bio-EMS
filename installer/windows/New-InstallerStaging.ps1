...
if ($ReleaseChannel -eq "Production") {
    ...
    foreach ($key in $keyring.keys) {
        if ($key.keyId -notmatch '^[a-zA-Z0-9][a-zA-Z0-9._-]{2,63}
...
Write-Host "BIO-EMS deterministic staging: PASS"
 -or
            [string]::IsNullOrWhiteSpace($key.publicKeyPem) -or
            $key.publicKeyPem -notmatch '-----BEGIN PUBLIC KEY-----' -or
            $key.status -notin @("active", "revoked") -or
            $keyIds.ContainsKey($key.keyId)) {
