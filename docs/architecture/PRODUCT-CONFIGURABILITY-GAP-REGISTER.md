...
| CFG-006 | Configuration RBAC            | BF-06 adds dedicated ADMIN-only recipient read/manage permissions                                      | Apply least-privilege authorization to every later configuration workflow                | PARTIAL / BF-06 VERIFIED       |
| CFG-007 | Configuration audit           | BF-06 recipient mutations add Site-scoped atomic safe evidence without contact values                  | Extend the same atomic rule to later controlled configuration families                   | PARTIAL / BF-06 VERIFIED       |
