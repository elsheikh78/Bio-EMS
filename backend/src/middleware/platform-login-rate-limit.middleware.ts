...
export function createPlatformLoginRateLimitMiddleware(
  now: () => number = () => Date.now(),
  ...
  return (request: Request, response: Response, next: NextFunction): void => {
    ...
    if (!current || current.resetAt <= currentTime) {
      ...
      attempts.set(client, { count: 1, resetAt: currentTime + WINDOW_MILLISECONDS });
      next();
      return;
    }
    ...
    if (current.count >= MAX_ATTEMPTS_PER_WINDOW) {
      ...
      throw new AppError("Too many authentication attempts", 429, "PLATFORM_LOGIN_RATE_LIMITED");
    }
...
export const platformLoginRateLimitMiddleware = createPlatformLoginRateLimitMiddleware();
