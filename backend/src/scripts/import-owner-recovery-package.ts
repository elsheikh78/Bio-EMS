...
import { z } from "zod";
import { applyOwnerRecovery, ownerRecoveryRequestSchema, type SignedOwnerRecoveryPackage, verifyOwnerRecoveryPackage } from "../modules/platform-auth/owner-recovery";
import { parseOwnerCommissioningTrustedKeyring, resolveOwnerCommissioningPublicKey } from "../modules/platform-auth/owner-commissioning-trust";

const identitySchema = z.object({ schemaVersion: z.literal(1), installationId: z.string().uuid() }).passthrough();

export async function runImportOwnerRecoveryPackage(environment: NodeJS.ProcessEnv = process.env, now = new Date()): Promise<number> {
  ...
  try {
    ...
    if (!identityPath || !requestPath || !packagePath) throw new Error("Owner recovery import paths are required");
    ...
    if (request.installationId !== identity.installationId) throw new Error("Recovery request belongs to a different installation");
    ...
    const keyring = parseOwnerCommissioningTrustedKeyring(JSON.parse(readFileSync(keyringPath, "utf8")));
...
if (require.main === module) void runImportOwnerRecoveryPackage().then((code) => { process.exitCode = code; });
