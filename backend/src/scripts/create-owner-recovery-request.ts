...
export async function runCreateOwnerRecoveryRequest(
  ...
  try {
    ...
    const request = ownerRecoveryRequestSchema.parse({
      schemaVersion: 1,
      recoveryId,
    ...
    database
      .prepare(
        `INSERT INTO password_recovery_requests (request_id, principal_type, username_hint, installation_id, challenge_hash, status, requested_at, expires_at) VALUES (?, 'SYSTEM_OWNER', NULL, ?, ?, 'PENDING', ?, ?)`
      )
      .run(
        recoveryId,
    ...
    database
      .prepare(
        `INSERT INTO password_recovery_audit (event_type, request_id, principal_type, actor_type, outcome, details_json) VALUES ('SYSTEM_OWNER_RECOVERY_REQUESTED', ?, 'SYSTEM_OWNER', 'LOCAL_INSTALLATION', 'SUCCESS', ?)`
      )
      .run(recoveryId, JSON.stringify({ installation_id: identity.installationId }));
...
if (require.main === module)
  void runCreateOwnerRecoveryRequest().then((code) => {
