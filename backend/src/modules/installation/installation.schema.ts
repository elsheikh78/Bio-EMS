...
const telemetry = z
  .object({
    ...
    alarmLow: z.number().nullable().optional(),
    warningHigh: z.number().nullable().optional(),
    alarmHigh: z.number().nullable().optional(),
...
const device = z
  .object({
    ...
    protocol: z.enum(["mqtt", "modbus", "other"]),
      ...
      .array(
        z
          .object({
            areaCode: code,
            telemetryCode: code,
            channel: z.number().int().min(0).max(1023),
          })
          .strict()
      )
      .min(1),
...
export const reviseInstallationSchema = z
  .object({
...
export const installationListQuerySchema = z
  .object({
    customerId: z
      .string()
      .regex(/^[1-9]\d*$/)
...
export const installationReceiptSchema = z
  ...
  .strict();
export const installationDecisionSchema = z
  .object({
