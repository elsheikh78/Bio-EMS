...
type Result = Awaited<ReturnType<TemperaturePerformanceReportService["preview"]>>;
...
function addTableHeader(
  ...
  for (const column of columns) {
    doc.rect(x, y, column.width, TABLE_HEADER_HEIGHT).stroke();

    doc.text(column.title, x + ROW_PADDING, y + 5, {
  ...
  doc.x = PAGE_MARGIN;
  doc.y = y + TABLE_HEADER_HEIGHT + COLUMN_GAP;
}
...
function addTableRow(doc: PDFKit.PDFDocument, values: string[], columns: Array<{ width: number }>) {
  ...
  values.forEach((value, index) => {
    ...
    doc.text(value, x + ROW_PADDING, y + ROW_PADDING, {
      width: column.width - ROW_PADDING * 2,
...
export function renderTemperaturePerformancePdf(
  ...
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      bufferPages: true,
      info: {
        Title: "BIO-EMS Temperature Performance",
        Subject: "Temperature Performance Report",
        Author: generatedBy,
        Creator: "BIO-EMS",
      },
    });
    ...
    doc.font("Helvetica-Bold").fontSize(20).text("BIO-EMS");

    doc.font("Helvetica-Bold").fontSize(16).text("Temperature Performance Report").moveDown(0.5);

    doc
      .font("Helvetica")
      .fontSize(9)
      .text("Controlled environmental monitoring output generated from BIO-EMS telemetry evidence.")
      .moveDown(1);
    ...
    summaryItems.forEach(([label, value], index) => {
      addSummaryBox(
        ...
        label,
        value,
        PAGE_MARGIN + (index % 3) * (boxWidth + boxGap),
        startY + Math.floor(index / 3) * 50,
        boxWidth
      );
    });
    ...
    addSectionTitle(doc, "Data quality");

    addKeyValue(doc, "Complete", result.quality.complete);

    addKeyValue(doc, "Warnings", result.quality.warnings.join(", "));

    addKeyValue(doc, "Unavailable sections", result.quality.unavailableSections.join(", "));

    addSectionTitle(doc, "Sensor performance");
    ...
    for (const sensor of result.sensors) {
      ...
      ensureVerticalSpace(doc, rowHeight(doc, values, columns), () => addTableHeader(doc, columns));
...
export function temperaturePerformancePdfFilename(result: Result) {
