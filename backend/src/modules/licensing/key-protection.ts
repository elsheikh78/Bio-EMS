...
export class WindowsDpapiKeyProtector implements KeyProtector {
  ...
  private execute(operation: "Protect" | "Unprotect", input: Buffer): Buffer {
    ...
    const script = `Add-Type -AssemblyName System.Security;$data=[Convert]::FromBase64String([Console]::In.ReadToEnd());$out=[System.Security.Cryptography.ProtectedData]::${operation}($data,$null,[System.Security.Cryptography.DataProtectionScope]::LocalMachine);[Console]::Out.Write([Convert]::ToBase64String($out))`;
