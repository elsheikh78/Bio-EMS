...
describe("DEP-01-03 protected configuration and service lifecycle source", () => {
  ...
  it("captures WinSW prestart stdout and stderr before the Node provisioner runs", () => {
    expect(lifecycle).toContain('$writer.WriteElementString("stdoutPath", "$logPath-prestart.stdout.log")');
    expect(lifecycle).toContain('$writer.WriteElementString("stderrPath", "$logPath-prestart.stderr.log")');
